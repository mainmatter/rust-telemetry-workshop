...
/// If everything runs smoothly, you will be asked if you want to move forward to the next koan.
#[derive(Parser)]
...
pub struct Command {
    ...
    ///
    /// E.g. `koans --path my_exercises` if `my_exercise` is a sub-directory of your current
    /// working directory.
    ///
...
fn run_tests(manifest_path: &Path, filter: Option<&str>) -> TestOutcome {
    ...
    // doesn't support it.
    let color_option = if cfg!(windows) && !Paint::enable_windows_ascii() {
        "never"
    } else {
        "always"
    };
